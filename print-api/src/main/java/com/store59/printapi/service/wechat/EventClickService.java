...
public class EventClickService {
	...
	Logger logger = Logger.getLogger(EventClickService.class);

	public void menuClick(EventMessage msg) {
		...
		if(msg.getEventKey().equals("bind")){
			new Thread(){
				public void run(){
					String uid=valueOpsCache.get(CommonConstant.WECHAT_OPENID_UID_PREFIX + msg.getFromUserName());
					Result<User> user=userApi.getUser(Long.parseLong(uid));
					TextMessage textMsg = new TextMessage(msg.getFromUserName(), "您的账号（"+user.getData().getUname()+"）已绑定成功，直接发送照片开始打印吧~");
					MessageAPI.messageCustomSend(tokenManager.getToken(appid), textMsg);
				}
			}.start();
			return;
	...
	private boolean isBinding(String openid) {
		...
		if (!StringUtil.isBlank(cashUid)) {
			logger.info("微信缓存用户openid信息："+CommonConstant.WECHAT_OPENID_UID_PREFIX + openid+";value:"+cashUid);
			return true;
		}
		// 查询用户是否绑定
