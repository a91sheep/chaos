 ...
 */
@Component
public class OrderDocReceiver {
    @Autowired
    private OrderCenterService    orderCenterService;
    @Autowired
    private PicCreateOrderService picCreateOrderService;

    private static Logger logger = LoggerFactory.getLogger(OrderDocReceiver.class);

    public void orderDocReceive(String data) {
        ...
        logger.info("文档订单:" + printOrder.getOrderId() + " 支付完成后推送消息到店长端!");
        picCreateOrderService.pushToPcConvert(printOrder);
        logger.info("文档订单:" + printOrder.getOrderId() + "推送成功!");
    }
}
