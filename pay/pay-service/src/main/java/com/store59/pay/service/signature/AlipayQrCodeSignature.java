...
@Component
public class AlipayQrCodeSignature implements Signature {

    private static final Logger logger = LoggerFactory.getLogger(AlipayQrCodeSignature.class);
    ...
    public String sign(Map<String, String> parameters) {
        ...
        logger.debug("【支付宝扫码支付】要签名的内容:{}", content);

        logger.debug("【支付宝扫码支付】签名key:{}", alipayConfig.getPrivateKey());

        //使用私钥签名
        String sign = RSA.sign(content, alipayConfig.getPrivateKey(), alipayConfig.getInputCharset());
        logger.debug("【支付宝扫码支付】计算出来的签名:{}", sign);
    ...
    public boolean verify(Map<String, String> parameters, String sign) {
        ...
        //Map转Url
        String content = MapUtils.toUrlWithSort(parameters);
        logger.debug("【支付宝扫码支付】要验证的签名内容:{}", content);

        logger.debug("【支付宝扫码支付】支付宝传过来的sign={}", sign);

        logger.debug("【支付宝扫码支付】验证签名使用的key={}", alipayConfig.getQrPublicKey());

        //支付宝消息校验
        boolean notifyIdFlag =  alipayMessageSignature.verify(parameters.get("notify_id"));
        //使用公钥验证
        boolean flag = RSA.verify(content, sign, alipayConfig.getQrPublicKey(), alipayConfig.getInputCharset());

        if(!notifyIdFlag){
            logger.error("【支付宝扫码支付】验证notifyId失败");
            return false;
        }

        if(!flag){
            logger.error("【支付宝扫码支付】验证签名失败");
            return false;
        }

        logger.debug("【支付宝扫码支付】验证notifyId和签名成功");
        return true;
    }
}
